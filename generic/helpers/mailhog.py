...
class MailhogApi:
    ...
    def get_token_by_change_password(self, login: str, attempt=50):
        if attempt == 0:
            raise AssertionError(f"Не удалось получить письмо с логином {login}")
        emails = self.get_api_v2_messages(limit=10).json()['items']
        for email in emails:
            user_data = json.loads(email['Content']['Body'])
            if login == user_data.get('Login') and user_data.get('ConfirmationLinkUri'):
                token = user_data['ConfirmationLinkUri'].split('/')[-1]
                print(token)
                return token
        time.sleep(2)
        print("Попытка получить письмо")
        return self.get_token_by_change_password(login=login, attempt=attempt - 1)


# if __name__ == '__main__':
#     MailhogApi().get_api_v2_messages(limit=10)
